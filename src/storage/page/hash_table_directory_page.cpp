...
uint32_t HashTableDirectoryPage::GetSplitImageIndex(uint32_t bucket_idx) {
  ...
  uint32_t split_image_index = high_bits | (bucket_idx & ((1 << (GetLocalDepth(bucket_idx) - 1)) - 1));
...
uint32_t HashTableDirectoryPage::GetLocalHighBit(uint32_t bucket_idx) {
  auto local_depth = GetLocalDepth(bucket_idx) - 1;
  return ((bucket_idx >> local_depth) + 1) << local_depth;
}
